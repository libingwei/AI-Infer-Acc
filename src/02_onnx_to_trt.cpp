...
int main(int argc, char** argv) {
    ...
    nvinfer1::INetworkDefinition* network = builder->createNetworkV2(0U); // 0U is for explicit batch
    nvinfer1::IBuilderConfig* config = builder->createBuilderConfig();
    nvonnxparser::IParser* parser = nvonnxparser::createParser(*network, gLogger);

    // 2. Parse the ONNX model file
    std::cout << "Parsing ONNX model: " << onnx_filename << std::endl;
    if (!parser->parseFromFile(onnx_filename, static_cast<int>(nvinfer1::ILogger::Severity::kWARNING))) {
